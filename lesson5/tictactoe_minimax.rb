...
class GAME
  def win?(player_object)
    #determines if the player object won the game
  end
  
  def over?
    #determines if the game is over because someone won or if it was a tie
  end
  ...
  def get_new_state(move)
    #returns a game object with new state of game after move
  end
  
  def active_turn(player_object)
    #returns the current player whoes turn it is
  end
end
...
class PerfectPlayer
  # choice variable, used to store where the player wants to move
  
  def make_move
    # needs to take choice and make a move with the choice stored in the choice variable
    # update board with new choice.
  end
